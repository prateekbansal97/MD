...
namespace md::Bonded
{
    double CMapGroup::return_energy_linear(const double phi_, const double psi_, const std::vector<double>& grid, const int resolution)
    {
        ...
        const double grid_spacing = (2.0 * M_PI) / resolution;
    ...
    std::pair<double, double> CMapGroup::return_gradient_linear(const double phi_, const double psi_, const std::vector<double>& grid, const int resolution)
    {
        double phi_shifted = phi_;
        double psi_shifted = psi_;
        ...
        const int i_next = (i + 1) % resolution;
        const int i_prev = (i - 1 + resolution) % resolution;

        const int j_next = (j + 1) % resolution;
        const int j_prev = (j - 1 + resolution) % resolution;

        // 4. Look up Energies
        // Grid structure: index = row * width + col
        // We vary i (phi) for the first gradient, keeping j (psi) constant
        const double e_phi_next = grid[i_next * resolution + j];
        const double e_phi_prev = grid[i_prev * resolution + j];

        // We vary j (psi) for the second gradient, keeping i (phi) constant
        const double e_psi_next = grid[i * resolution + j_next];
        const double e_psi_prev = grid[i * resolution + j_prev];
    ...
    double CMapGroup::return_energy_bicubic(const double phi, const double psi, const int resolution, const std::vector<double>& coeffs, const std::vector<double>& grid_energies) const
    {
        ...
        const double grid_pos_phi = phi_shifted / grid_spacing;
        const double grid_pos_psi = psi_shifted / grid_spacing;
        ...
        const double u = grid_pos_phi - static_cast<double>(row_grid);
        const double v = grid_pos_psi - static_cast<double>(col_grid);
        ...
        for (const double i : u_pow) {
            for (const double j : v_pow) {
                final_energy += coeffs[coeff_start_index + k] * i * j;
                k++;
            }
        }
    ...
    std::pair<double, double> CMapGroup::return_gradient_bicubic(const double phi, const double psi, const int resolution, const std::vector<double>& coeffs, const std::vector<double>& grid_energies) const
    {
        ...
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                const double coeff = coeffs[coeff_start_index + k];
        ...
        const double inv_spacing = 1.0 / grid_spacing;
